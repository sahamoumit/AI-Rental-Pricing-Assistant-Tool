...
- **Frontend** — single-page UI (Tailwind HTML + a small vanilla-JS layer in `frontend/js/`). Fully wired to the backend for Milestones 1–7: property list + selected-property details, top-5 comparables on demand, a rent recommendation card (rent, price range, confidence pill, "why this price" factor list, timestamp) driven by **Generate AI Recommendation**, an alternative **Run via Pricing Agent** path that surfaces a numbered agent-reasoning trace alongside the same card, analyst-driven **Recalculate Recommendation** (toggle comp checkboxes to include/exclude and re-price with the same math), a live chat panel that asks follow-up questions about the current recommendation, and a feedback form that captures the analyst's take on each recommendation. Every panel talks to a live endpoint — no placeholders remain.
- **Backend** — FastAPI service. Milestones 1–7 are live: loads the CSVs at startup, serves property list/detail endpoints, returns the top-5 comparable properties for any target via a deterministic weighted similarity score, produces a rent recommendation with confidence and price range via a similarity-weighted comp model with area/amenity/location adjustments, re-computes that recommendation over an analyst-supplied comp set, exposes a Pricing Agent that orchestrates comparable retrieval + pricing (retrying with a broader comp set when confidence is weak) without duplicating the pricing math, answers analyst questions with a local Llama model that grounds every response in the target property + current recommendation + chosen comparables, and appends analyst feedback (property, rent, chosen comps, free text, ISO 8601 UTC timestamp) to `data/analyst_feedback.csv`.
- **AI layer** — comparable retrieval + a pricing model (both shipped as deterministic services in M2/M3, exposed to analyst edits in M4). M7 wraps them in a `PricingAgent` that decides how many comps to consider and when to retry, then delegates the final rent math back to `services/pricing.py` — the pricing formula stays in exactly one place. A local Llama model (via Ollama's HTTP API) powers the conversational Q&A in M5. Feedback captured in M6 is stored append-only for a future Learning Agent to consume. Remaining agents (Property Intelligence, Comparable, Conversation, Learning, Orchestrator) still planned as wrappers over these services.
...
│       ├── agents/
│       │   └── pricing_agent.py     # PricingAgent — orchestrates similarity + pricing services
│       └── prompts/
...
| `POST` | `/agent/pricing` | 7 | Pricing Agent orchestration over the Comparable + Pricing services. Body: `{"property_id": "P0001"}`. Loads the target, fetches the top-5 comps, and if their mean similarity falls below the low-confidence threshold (default `0.55`), retries once with `n=10` and passes those exact comp ids to the pricing service. Final rent math is delegated to `services/pricing.py` — the agent never duplicates the formula. Response mirrors `/recommend` (`recommended_rent`, `confidence`, `price_range`, `pricing_factors`, `comparables_used`) plus `agent_reasoning: string[]` — a numbered trace of what the agent did (comp count, mean similarity, retry-or-not, delegate). Stateless. Returns `400` on missing/bad body, `404` if the target is unknown. |

## Future Improvements

- **Learning loop** — capture is live. Next step is a read-only Learning Agent that classifies and aggregates `analyst_feedback.csv` (over-priced / under-priced / neutral by locality and property type) and exposes it via `GET /learning/insights`. Pricing stays deterministic — any feedback influence on a recommendation would land later as an explicit, opt-in adjustment that shows up in `pricing_factors` alongside area/amenity/location, never as a silent nudge.
