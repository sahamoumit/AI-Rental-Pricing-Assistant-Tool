...
- ⏳ **Not yet built:** Pydantic response models; Property Intelligence, Pricing, Conversation, Learning, Orchestrator agents (the Comparable *Agent* wrapper and Pricing *Agent* wrapper are also still TBD — M2 and M3 shipped the underlying services only); `Recalculate Recommendation` (comp toggle → new price), chat, and feedback endpoints + their frontend wiring.
